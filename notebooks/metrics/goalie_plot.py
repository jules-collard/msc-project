...
def _(
    ...
    p = (
        ...
        + p9.scale_size_continuous(breaks=[0.25, 0.5, 0.75], labels=lambda x: [f"±{label}" for label in x])
        + p9.scale_alpha_continuous(breaks=[0.25, 0.5, 0.75], labels=lambda x: [f"±{label}" for label in x])
        + p9.scale_x_reverse()
