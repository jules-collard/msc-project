...
app = marimo.App(width="medium", app_title="Shot Dashboard")
...
with app.setup:
    ...
    from plotting.rink import geom_net, geom_rink
...
def _(games, player_mapping, smt_id, sportlogiq_id):
    ...
    shot_data = (
        ...
        .with_columns(
            ...
            details = pl.format("{} {} - {}mph - {}", c('player_first_name'), c('player_last_name'), c('shot_speed').round(1).cast(pl.String).fill_null("N/A"), c('result'))
        )
        .with_columns(
...
@app.cell
def _(puck_tracking, shap, shot_data, shot_selector):
...
@app.cell
def _(shot_plot_data, shot_puck_tracking, shot_tracking):
    rink_plot = (
        ggplot(shot_plot_data, aes(x='shot_x', y='shot_y'))
        + geom_rink(fill='white')
        + gg.geom_point(
            aes(x='x_adj', y='y_adj', fill='team_source', tooltip="PlayerName", hover_group="PlayerName"),
            size=8,
            data=shot_tracking,
            inherit_aes=False
        )
...
def _(base_value, shot_shap):
    if shot_shap.height > 0:
        ...
        shap_plot = (
            ...
            + gg.geom_hline(yintercept=0, alpha=0.6)
            + gg.geom_rect(
                aes(
                    xmin="x_id - 0.4", 
                    xmax="x_id + 0.4", 
...
@app.cell
def _(game_selector, goal_plot, rink_plot, shap_plot, shot_selector):
    mo.vstack([
        mo.hstack([
